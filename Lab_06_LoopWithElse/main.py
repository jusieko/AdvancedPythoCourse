...
wewnątrz pętli while dodaj blok try/except, który w przypadku błędu zakończy wykonywanie pętli, wyświetlając komunikat o błędzie

zakończ pętlę while poleceniem, które wykona się tylko wtedy gdy pętla nie została w żaden sposób przerwana. Wyświetl tu komunikat o powodzeniu
...
data_dir = 'c:\\users\\janos\\projektBartek\\'
pages = [
...
for page in pages:
    try:
        ...
        print(path)
        #file = open('c:\\users\\janos\\projektBartek\\{}'.format(page['name']+'.html'), 'x')
